...
namespace ns3
{
	...
	{
		static TypeId tid =
				TypeId("ns3::StaWifiMac").SetParent<RegularWifiMac>().SetGroupName(
						"Wifi").AddConstructor<StaWifiMac>().AddAttribute(
						"ProbeRequestTimeout",
						...
						MakeTimeChecker()).AddAttribute("RawDuration",
						"The duration of one RAW group.",
						TimeValue(MicroSeconds(102400)),
						MakeTimeAccessor(&StaWifiMac::GetRawDuration,
								&StaWifiMac::SetRawDuration), MakeTimeChecker()).AddAttribute(
						"MaxMissedBeacons",
						"Number of beacons which much be consecutively missed before "
								"we attempt to restart association.",
						UintegerValue(10),
						MakeUintegerAccessor(&StaWifiMac::m_maxMissedBeacons),
						MakeUintegerChecker<uint32_t>()).AddAttribute("StaType",
						...
						"ns3::Mac48Address::TracedCallback").AddTraceSource(
	...
	StaWifiMac::StaWifiMac()
			: m_state(BEACON_MISSED), m_probeRequestEvent(), m_assocRequestEvent(), m_disassocRequestEvent(), m_beaconWatchdogEnd(
					Seconds(0.0))
	{
	...
	void StaWifiMac::GoToSleepNextTIM(S1gBeaconHeader beacon) //to do, merge with GoToSleepCurrentTIM
	{
		...
		Simulator::Schedule(
				MicroSeconds(
						beacon.GetBeaconCompatibility().GetBeaconInterval()),
				&StaWifiMac::GoToSleep, this, sleepTime);
		//GoToSleep (MicroSeconds (beacon.GetBeaconCompatibility().GetBeaconInterval () * BeaconNumForTIM));
	}
	...
	void StaWifiMac::GoToSleep(Time sleeptime)
	{
		...
		{
			NS_LOG_DEBUG(
					"At " << Simulator::Now().GetSeconds() << " s AID " << this->GetAID() << " switches to SLEEP. Schedule wake-up after " << sleeptime.GetSeconds() << " s.");
	...
	void StaWifiMac::S1gBeaconReceived(S1gBeaconHeader beacon)
	{
		...
		{
			m_outsideRawEvent = Simulator::Schedule(m_lastRawDurationus,
					&StaWifiMac::OutsideRawStartBackoff, this);
		...
		else if (m_rawStart && m_inRawGroup && !m_pagedStaRaw)
		{
			...
			m_edca.find(AC_BE)->second->AccessAllowedIfRaw(false);
			m_edca.find(AC_BK)->second->AccessAllowedIfRaw(false);
			Simulator::Schedule(m_statSlotStart,
					&StaWifiMac::RawSlotStartBackoff, this);
		}
	...
	void StaWifiMac::RawSlotStartBackoff(void)
	{
		...
		m_insideBackoffEvent = Simulator::Schedule(m_currentslotDuration,
				&StaWifiMac::InsideBackoff, this);
		m_pspollDca->AccessAllowedIfRaw(true);
		m_dca->AccessAllowedIfRaw(true);
	...
	void StaWifiMac::TxOk(const WifiMacHeader &hdr)
	{
		...
		{
			NS_LOG_UNCOND(
					GetAddress () << " disassociation request is received bt " << hdr.GetAddr1 ());
	...
	void StaWifiMac::SendDisAssociationRequest(void)
	{
		...
		if (m_s1gSupported)
		{
			...
			NS_LOG_UNCOND(
					GetAddress () << " StaWifiMac::SendDisAssociationRequest ");
	...
	void StaWifiMac::Enqueue(Ptr<const Packet> packet, Mac48Address to)
	{
		...
		if (!IsAssociated())
		{
			...
			TryToEnsureAssociated();
			NS_LOG_UNCOND(
					m_low->GetAddress () << "  cannot send since not associated");
			return;
		}
		WifiMacHeader hdr;
	...
	void StaWifiMac::Receive(Ptr<Packet> packet, const WifiMacHeader *hdr)
	{
		...
		else if (hdr->IsS1gBeacon())
		{
			...
			if (goodBeacon)
			{
				...
				{
					uint8_t rawtypeindex = rawassign[raw_index
							* rawAssignment_len + 0] & 0x07;
					...
					uint8_t pageindex = rawassign[raw_index * rawAssignment_len
							+ 3] & 0x03;

					uint16_t m_rawslot;
					m_rawslot =
							(uint16_t(
									rawassign[raw_index * rawAssignment_len + 2])
									<< 8)
									| (uint16_t(
											rawassign[raw_index
													* rawAssignment_len + 1]));
					uint8_t m_SlotFormat = uint8_t(m_rawslot >> 15) & 0x0001;
					uint8_t m_slotCrossBoundary = uint8_t(m_rawslot >> 14)
							& 0x0002;

					m_currentRAW_start = m_currentRAW_start
							+ (500 + m_slotDurationCount * 120) * m_slotNum;
					...
					m_slotDuration = MicroSeconds(
							500 + m_slotDurationCount * 120);
					m_lastRawDurationus = m_lastRawDurationus
							+ m_slotDuration * m_slotNum;

					if (pageindex == ((GetAID() >> 11) & 0x0003)) //in the page indexed
					{

						uint8_t rawgroup_l = rawassign[raw_index
								* rawAssignment_len + 3];
						uint8_t rawgroup_m = rawassign[raw_index
								* rawAssignment_len + 4];
						uint8_t rawgroup_h = rawassign[raw_index
								* rawAssignment_len + 5];
						uint32_t rawgroup = (uint32_t(
								rawassign[raw_index * rawAssignment_len + 5])
								<< 16)
								| (uint32_t(
										rawassign[raw_index * rawAssignment_len
												+ 4]) << 8)
								| uint32_t(
										rawassign[raw_index * rawAssignment_len
												+ 3]);
						...
						statRawSlot = ((GetAID() & 0x03ff) + offset)
								% m_slotNum;

						if ((raw_start <= (GetAID() & 0x03ff))
								&& ((GetAID() & 0x03ff) <= raw_end))
						{
							m_statSlotStart = MicroSeconds(
									(500 + m_slotDurationCount * 120)
											* statRawSlot + m_currentRAW_start);
							SetInRAWgroup();
		...
		else if (hdr->IsProbeResp())
		{
			if (m_state == WAIT_PROBE_RESP)
			{
				...
				for (uint32_t i = 0; i < m_phy->GetNBssMembershipSelectors();
						i++)
				{
					uint32_t selector = m_phy->GetBssMembershipSelector(i);
					if (!rates.IsSupportedRate(selector))
		...
		else if (hdr->IsAssocResp())
		{
			if (m_state == WAIT_ASSOC_RESP)
			{
				...
				if (assocResp.GetStatusCode().IsSuccess())
				{
					...
					NS_LOG_DEBUG(
							"[" << this->GetAddress() <<"] is associated and has AID = " << this->GetAID());
					...
					if (m_htSupported)
					{
						...
						for (uint32_t i = 0; i < m_phy->GetNMcs(); i++)
						{
							...
							{
								m_stationManager->AddSupportedMcs(
										hdr->GetAddr2(), mcs);
